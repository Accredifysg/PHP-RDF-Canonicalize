...
> **Status: 0.1.0 — initial extraction.** This package was lift-and-shifted
> out of [`accredifysg/verifiable-credentials-php`](https://github.com/accredifysg/verifiable-credentials-php),
> mirroring how `accredifysg/php-json-ld` was extracted earlier. The canonical
> output is **byte-for-byte identical** to the implementation VC ships, because
> VC's `eddsa-rdfc-2022` and `ecdsa-sd-2023` signatures are computed over it —
> see [Parity](#parity-with-verifiable-credentials-php).
...
- [x] RDF Dataset Canonicalization (RDFC-1.0) over N-Quads, using SHA-256 (the
      algorithm's default hash).

Out of scope for the 0.x line: JSON-LD → N-Quads conversion (use php-json-ld),
the optional SHA-384 hash profile, and removal of duplicate input quads — see
...
| ---------------- | --------: | ------: | ------------------------------ |
| Eval             |        64 |      60 | 4 documented gaps              |
| Map (identifiers)|        21 |      20 | 1 documented gap (SHA-384)     |
...
trace to 3 root causes, none of which affect the VC pipeline (php-json-ld's
`toRdf` emits SHA-256-friendly, de-duplicated, spec-escaped N-Quads):

- **`#test060c` — N-Quads escaping.** The lifted parser/serializer doesn't
  implement the full N-Quads `ECHAR`/`UCHAR` escaping grammar.
- **`#test075c` / `#test075m` — SHA-384 profile.** `RDFC10` hardcodes SHA-256
  (the RDFC-1.0 default); the optional SHA-384 profile is unsupported.
